...
class CovPlugin(object):
    ...
    this test process is centralised, a distributed main or a
    ...
    def __init__(self, options, pluginmanager, start=True):
        ...
        if is_dist and start:
            self.start(engine.DistMain)
        elif start:
            self.start(engine.Central)

        # subordinate is started in pytest hook
    ...
    def _is_subordinate(self, session):
        return hasattr(session.config, 'subordinateinput')
    ...
    def pytest_sessionstart(self, session):
        ...
        self.pid = os.getpid()
        if self._is_subordinate(session):
            nodeid = session.config.subordinateinput.get('subordinateid',
                                                   getattr(session, 'nodeid'))
            self.start(engine.DistSubordinate, session.config, nodeid)
    ...
    def pytest_runtestloop(self, session):
        ...
        if not self._is_subordinate(session) and self._should_report():
            try:
